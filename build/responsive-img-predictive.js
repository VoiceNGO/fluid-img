...
(() => {
  ...
  var PredictiveGenerator = class extends BaseGenerator {
    ...
    async generateSeamBatch() {
      ...
      const seams = [];
      const energyMapData = energyMap.energyMap;
      const topEnergyCount = 10;
      const energyPenaltyMultiplier = 50;
      const addEnergyWithTopTracking = (seam, pixelEnergy) => {
  ...
  var Generator = false ? FullGenerator : false ? CachedGenerator : true ? PredictiveGenerator : RandomGenerator;
  ...
  var componentName = "responsive-img" + (false ? "" : `-${"predictive"}`);
