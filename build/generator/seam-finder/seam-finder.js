...
export class SeamFinder {
    ...
    #height;
    #midPointRow1;
    #cumulativeEnergyTopDown;
    #parentPointersTopDown;
    #cumulativeEnergyBottomUp;
    #parentPointersBottomUp;
    #lastFoundSeam = null;
    constructor(imageDataRaw, width, height) {
        this.#width = width;
        this.#height = height;
        const imgData = { data: imageDataRaw, width, height, colorSpace: 'srgb' };
    ...
    #fillDPTable(dpTable, parentPointerTable, imageLoopStartRow, imageLoopEndRowInclusive, yStep, dpTableIndexOffset) {
        for (let currentImageY = imageLoopStartRow; yStep === 1
            ? currentImageY <= imageLoopEndRowInclusive
            : currentImageY >= imageLoopEndRowInclusive; currentImageY += yStep) {
    ...
    #calculateTopDownDP() {
        ...
        if (this.#midPointRow1 >= 1) {
            this.#fillDPTable(this.#cumulativeEnergyTopDown, this.#parentPointersTopDown, 1, this.#midPointRow1, 1, 0);
        }
    }
    #calculateBottomUpDP() {
        const localMidPointRow2 = this.#midPointRow1 + 1;
        ...
        if (lastImageRow - 1 >= localMidPointRow2) {
            this.#fillDPTable(this.#cumulativeEnergyBottomUp, this.#parentPointersBottomUp, lastImageRow - 1, localMidPointRow2, -1, localMidPointRow2);
        }
    }
    #findAndTraceSeam() {
